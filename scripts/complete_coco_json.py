# Adding some missing information needed for the model to train to coco
# Note: The area is incorrectly calculated using the bounding box instead of the semantic mask.
import json
import os
import argparse
...
def complete_coco_json(args):
    ...
    if (
        "categories" not in coco_annotation_dict.keys()
        or len(coco_annotation_dict["categories"]) == 0
    ):
    ...
    with open(
        os.path.join(args.input_dir, "coco_annotations_processed.json"), "w"
    ) as f:
